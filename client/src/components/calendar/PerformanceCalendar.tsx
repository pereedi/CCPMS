...
import { Calendar as CalendarIcon, ChevronLeft, ChevronRight, FileText, CheckCircle2, Target, Eye, X, AlertTriangle } from 'lucide-react';
...
export const PerformanceCalendar: React.FC = () => {
  ...
  const fetchCalendarRecords = async () => {
    try {
      ...
      console.warn('Calendar records fetch error', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getReportsForDay = (dayNum: number) => {
    ...
    return records.filter((r) => {
      if (r.submittedAt && r.submittedAt.startsWith(targetDateStr)) return true;
      if (r.reportDate && r.reportDate.startsWith(targetDateStr)) return true;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
              Track daily, monthly, and quarterly directorate report submissions, review dates & resubmissions
            </p>
          </div>
        </div>
                  ...
                  {dayReports.map((r, rIdx) => {
                    ...
                    return (
                      <div key={rIdx} style={{
                        ...
                        padding: '3px 6px',
                        borderRadius: '4px',
                        background: isApp ? 'rgba(16, 185, 129, 0.2)' : isRet ? 'rgba(244, 63, 94, 0.2)' : 'rgba(245, 158, 11, 0.2)',
                        border: isApp ? '1px solid rgba(16, 185, 129, 0.4)' : isRet ? '1px solid rgba(244, 63, 94, 0.4)' : '1px solid rgba(245, 158, 11, 0.4)',
                        fontSize: '0.68rem',
                        color: isApp ? '#34d399' : isRet ? '#f87171' : '#fbbf24',
                        fontWeight: 700,
                        whiteSpace: 'nowrap',
                        overflow: 'hidden',
      ...
      {selectedDayReports && (
        <div style={{
          ...
          <div className="glass-panel animate-fade-in" style={{ width: '520px', padding: '24px', borderRadius: '16px', background: '#111827' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <CalendarIcon style={{ width: '20px', height: '20px', color: '#60a5fa' }} />
                <h3 style={{ fontSize: '1.1rem', fontWeight: 700, color: '#ffffff' }}>
                  Submissions for {selectedDayReports.dateStr}
                </h3>
              </div>
            ...
            {selectedDayReports.items.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {selectedDayReports.items.map((item, idx) => (
                  <div key={idx} style={{
                    padding: '14px', borderRadius: '10px', background: 'rgba(15, 23, 42, 0.6)', border: '1px solid var(--border-color)'
                  }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
                      <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#ffffff' }}>{item.title || item.period}</span>
                      <span className="badge badge-role" style={{ fontSize: '0.7rem' }}>{item.status}</span>
                    </div>
                    <div style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', marginTop: '4px' }}>{item.summary}</div>
                    <div style={{ fontSize: '0.72rem', color: '#60a5fa', marginTop: '6px' }}>
                      Directorate: {item.directorateName || item.username} • Submitted by @{item.username}
                    </div>
                  </div>
                ))}
              </div>
